...
class MatchDetailViewModel @Inject constructor(
    ...
    val effects: Flow<List<MatchDetailsEffect>> =
        _effects.onStart { emit(listOf(MatchDetailsEffect.Ready(::onReady))) }


    private fun onReady(matchId: Int) {
    ...
    private suspend fun fetch(id: Int, onSuccess: () -> Unit = {}) {
        ...
        _state.update { prevState ->
            prevState.copy(
                matchId = id,
                data = result?.let { PullState.Pulled(result) } ?: PullState.Failed
            ).also {
                onSuccess()
    ...
    private fun mapState(state: MatchDetailState): MatchViewState = MatchViewState(
